...
const EditNanny_schedule = ({ route, navigation }: any) => {
    ...
    const saveChanges = () => {
        ...
        const updated = { day: selectedDay, start_time: convertFormat(startTime), end_time: convertFormat(endTime) };
        const value = finalShifts.filter(s => s.day !== item.day && s.start_time !== item.start_time && s.end_time !== item.end_time);
        value.push(updated)
    ...
    return (
        <NativeBaseProvider>
            <Box m='5%'>
                <Heading mb={8}>Edit specific schedule</Heading>
                <Center mt={6}>
                    <Text mb={6} bold fontSize={18}>{`${selectedDay.substring(0, 3)} : ${convertFormat(startTime)} - ${convertFormat(endTime)}`}</Text>
                    <Box maxW="300">
                        <Center>
                        ...
                        <Center>
                            <FormControl isRequired>
                                <Button variant='link' onPress={() => setStartTimeOpen(true)}>Modify start time</Button>
                                <DatePicker
                                    modal
                                    mode='time'
                                    open={startTimeOpen}
                        ...
                        <Center>
                            <FormControl isRequired isInvalid={inputErrors.type == 'START_END_SET_ERROR'}>
                                <Button variant='link' onPress={() => setEndTimeOpen(true)}>Modify end time</Button>
                                <DatePicker
                                    modal
                                    mode='time'
                                    open={endTimeOpen}
                    ...
                    <HStack space={2} mt={10}>
                        <Button borderRadius={20} onPress={() => navigation.goBack()}>Back</Button>
                        <Button borderRadius={20} onPress={saveChanges}>Save</Button>
                    </HStack>
                </Center>
            </Box>
        </NativeBaseProvider>
